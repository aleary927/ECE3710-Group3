...
# extra timer instructions
counter_insts = {'MSCP', 'MSCR', 'MSCG'}
...
def assemble(filename: str):
    ...
    for i, x in enumerate(f):
        ...
        if len(parts) > 0 and line[0] != '.' and line[0] != '`':
            ...
            # added for counter instructions
            elif instr in counter_insts: # -----------------------------------------------------------------------------
                if instr == 'MSCP': 
                    # check for immediate operand of 0 or 1
                    if len(parts) != 1: 
                        sys.exit(f'ERROR: Wrong number of args on line {i+1} in instruction {x}\n\tExpected: 1, Found: {len(parts)}')
                    imm = parts[0]
                    if imm[0] == '$':
                        ...
                        sys.exit(f'ERROR: Badly formatted imm on line {i+1} in instruction {x}'
                                +f'\n\tExpected imm to start with: \'$\', but found: \'{imm[0]}\'')
                    if parsed_imm not in [1, 0]: 
                        print('WARNING: immediate value of MSCP insruction is not 0 or 1 on line {i+1}')
                    formatted_imm = f'{(parsed_imm & 0xFF):02X}'
                    wf.write(inst_codes[instr] + '0' + formatted_imm + '\n')
                elif instr == 'MSCR': 
                    # check for no operands
                    if len(parts) != 0:
                        sys.exit(f'ERROR: Wrong number of args on line {i+1} in instruction {x}\n\tExpected: 0, Found: {len(parts)}')
                    wf.write(inst_codes[instr] + '0' + '0' + '0' + '\n')
                elif instr == 'MSCG':
                    # check for single, register operand
                    if len(parts) != 1: 
                        sys.exit(f'ERROR: Wrong number of args on line {i+1} in instruction{x}\n\tExpected: 1, Found: {len(parts)}')
                    r_dest = parts[0];
                    if r_dest not in reg_codes: 
                        sys.exit(f'ERROR: Unrecognized register on line {i+1} in instruction {x}')
                    wf.write(inst_codes[instr] + reg_codes[r_dest] + '0' + '0' + '\n')
                else: 
                    sys.exit(f'ERROR: Unrecognized counter instruction on line {i+1}; instruction: {x}')
            elif instr == 'NOP':
